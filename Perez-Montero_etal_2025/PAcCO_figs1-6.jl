...
hidedecorations!(ax)
hidespines!(ax)
lines!(ax, df1["time"] ./ 1e3, zthr .* NaN, color=:darkred, linewidth=2, strokecolor=:darkred, strokewidth=0.0001, label=L"$z_\mathrm{thr}$")
for r in eachindex(runs2plot)
    dfr = NCDataset("data/runs/$(runs2plot[r]).nc")
    lines!(ax, dfr["time"] ./ 1e3, dfr["Tsurf"] .- dfr["Tref"] .* NaN, color=colors[r], linewidth=4, label=labels[r])
end
axislegend(ax, framevisible=false, position=(:center, :center), labelsize=25, nbanks=3)
...
fig = Figure(resolution=(1500, 600), fonts=(; regular="TeX"), fontsize=28)
